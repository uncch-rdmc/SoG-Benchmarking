...
# 2024-01-18 update edition
# mainly copied from the edition of 2023-11-29
# differences are data themselves not script
###############################################################################
...
# =============================================================================
# !!!!!!!!!!!!! important !!!!!!!!!!!!!
# Original files provided by SoG must be renamed as follows:
# Data file
# original: OdumFormat.xlsx
# required: bm_data.xlsx
# 
# variable-name list-file
# original: MetricNames.xlsx
# required:# vnl.xlsx
# 
# 

# The 2nd file may not be sent and if so recycle the latest one
# as of 2024-02-02, `all_service_info.xlsx` in the previous working directory 
#  must be copied to the current workding directory if it is not provided.

#------------------------------------------------------------------------------
# Part 1: Benchmark Data
#------------------------------------------------------------------------------
# step 0: set the storage directory as the current working directory, such as, 
# setwd("~/Documents/experiments/visualization/benchmarking/202x-xx-xx")
# 
# Required libraries 
# library(tidyverse)
# library(readxl)
...
# 2023-10 update
df_service_all <- df_service_all |> dplyr::mutate_at(c('Year'), as.integer)
df_service_all <- df_service_all |> dplyr::mutate_at(c('Value'), as.double)
df_service_all
# -----------------------------------------------------------------------------
...
# 2023-11-29 update: the name of the sub-directory was changed to `raw data`
all_label_def_data_csv <- 
  readr::read_csv(col_names = c("var_name", "var_label", "var_def"),
skip = 1,                  file="./raw data/MetricNames.csv")
all_label_def_data_csv
...
# -----------------------------------------------------------------------------
# modified as of 2024-01-19
all_service_abbrev_to_full <- 
  readxl::read_excel("all_service_info.xlsx",
...
# 
# 
# update the date-token in app.R around at line 2481
# 
# 
###############################################################################
